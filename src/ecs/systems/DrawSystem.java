...
public class DrawSystem extends System {
  ...
  public void execute() {
        ...
        .forEach(
            entity -> {
              ...
              try {
                ...
                dc.nextImage();
                entity.fetch(HealthComponent.class).ifPresent(hc ->  {
    ...
    private void renderHealth(HealthComponent hc, PositionComponent pc) {
        ...
        for (int segment = 0; segment < hc.getMaxHealth(); segment++) {
            if (segment <hc.getCurrentHealth()) {
                renderSegment(pc.getX(),pc.getY(),segmentWidth,healthBarHeight,Color.RED, segment*segmentWidth);
    ...
    private void renderSegment(double x, double y, double width, double height, Color color, double offset) {
      Rectangle2D rect = new Rectangle2D.Double(x * GameLoop.RENDERSCALE + offset,y * GameLoop.RENDERSCALE+16, width, height);
    ...
    private void renderOutline(double x, double y, double height, Color color) {
        Rectangle2D rect = new Rectangle2D.Double(x * GameLoop.RENDERSCALE,y * GameLoop.RENDERSCALE+16, 16, height);
        GameLoop.graphics.setColor(color);
